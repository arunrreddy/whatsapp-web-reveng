...
from whatsapp_defines import WAWebMessageInfo, WAMetrics, WAFlags
import pyqrcode
import curve25519
import websocket
import traceback
...
def HKDF(key, length, appInfo=""):
    ...
    while len(keyStream) < length:
        keyBlock = hmac.new(key, msg=keyBlock+appInfo +
                            chr(blockIndex), digestmod=hashlib.sha256).digest()
        blockIndex += 1
        keyStream += keyBlock
...
def WhatsAppEncrypt(encKey, macKey, plaintext):
    ...
    # this may need padding to 64 byte boundary
...
class WhatsAppWebClient:
    ...
    def onMessage(self, ws, message):
        try:
            eprint('incoming message', message)
            messageSplit = message.split(",", 1)
            eprint('message split', messageSplit)
            messageTag = messageSplit[0]
            eprint('message tag', messageSplit[0])
            ...
            if messageTag in self.messageQueue:											# when the server responds to a client's message
                ...
                if pend["desc"] == "_status":
                    ...
                    self.loginInfo["serverRef"] = json.loads(messageContent)[
                        "ref"]
                    ...
                    pyqrcode.create(qrCodeContents, error='L').svg(
                        svgBuffer, scale=6, background="rgba(0,0,0,0.0)", module_color="#122E31", quiet_zone=0)
                    if "callback" in pend and pend["callback"] is not None and "func" in pend["callback"] and pend["callback"]["func"] is not None and "tag" in pend["callback"] and pend["callback"]["tag"] is not None:
                        pend["callback"]["func"]({"type": "generated_qr_code", "image": "data:image/svg+xml;base64," + base64.b64encode(
                            svgBuffer.getvalue()), "content": qrCodeContents}, pend["callback"])
            else:
                try:
                    # try reading as json
                    ...
                    if messageContent != "":
                        ...
                        try:
                            processedData = whatsappReadBinary(
                                decryptedMessage, True)
                            messageType = "binary"
                        except:
                            processedData = {
                        ...
                        self.onMessageCallback["func"](message, self.onMessageCallback, {
                            "message_type": "json"
                        })
                else:
                    self.onMessageCallback["func"](jsonObj, self.onMessageCallback, {
                                                   "message_type": "json"})
                    # check if the result is an array
                    if isinstance(jsonObj, list) and len(jsonObj) > 0:
                        eprint(json.dumps(jsonObj))
                        if jsonObj[0] == "Conn":
                            Timer(25, lambda: self.activeWs.send(
                                '?,,')).start()  # Keepalive Request
                            ...
                            hmacValidation = HmacSha256(
                                sse[32:64], self.connInfo["secret"][:32] + self.connInfo["secret"][64:])
                            if hmacValidation != self.connInfo["secret"][32:64]:
                                raise ValueError("Hmac mismatch")

                            keysEncrypted = sse[64:] + \
                            ...
                            eprint(
                                "logged in as " + jsonObj[1]["pushname"] + " (" + jsonObj[1]["wid"] + ")")
    ...
    def connect(self):
        self.activeWs = websocket.WebSocketApp("wss://w1.web.whatsapp.com/ws",
                                               on_message=lambda ws, message: self.onMessage(
                                                   ws, message),
                                               on_error=lambda ws, error: self.onError(
                                                   ws, error),
    ...
    def getLoginInfo(self, callback):
        callback["func"](
            {"type": "login_info", "data": self.loginInfo}, callback)

    def getConnectionInfo(self, callback):
        callback["func"]({"type": "connection_info",
                          "data": self.connInfo}, callback)
    ...
    def sendTextMessage(self, number, text):
        ...
        eprint("Message Data", msgData)
        finalCipher = AESEncrypt(
            self.loginInfo["key"]["encKey"], whatsappWriteBinary(msgData))
        finalHash = HmacSha256(self.loginInfo["key"]["macKey"], finalCipher)
        eprint('flag', bytearray(to_bytes(WAFlags.IGNORE, 1)))
        payload = bytearray(messageId) + bytearray(",") + bytearray(
            to_bytes(WAMetrics.MESSAGE, 1)) + bytearray(to_bytes(WAFlags.IGNORE, 1)) + finalHash + finalCipher
    ...
    def status(self, callback=None):
        if self.activeWs is not None:
            messageTag = str(getTimestamp())
            self.messageQueue[messageTag] = {
                "desc": "_status", "callback": callback}
